...
class TinychatRTCClient(object):
    ...
    def console_write(self, color, message):
        ...
        if config.CONSOLE_COLORS:
            msg = COLOR['white'] + Style.RESET_ALL + color + message
        else:
            msg = message
        try:
            print(msg)
        ...
        if config.CHAT_LOGGING:
            write_to_log(message, self.room_name)
    ...
    def connect(self):
        ...
        # This is the only change. It returns token['result'] which does fuck all
        # and token['endpoint'] which is the websocket we need. 
        token = requests.get('https://tinychat.com/api/v1.0/room/token/' + self.account + '?').json()
...
#    def on_userlist(self, user_info):   #ORIGINAL
#
#        if user_info['handle'] != self.client_id:
#            _user = self.users.add(user_info)
#            if _user.is_owner:
#                _user.user_level = 1
#                self.console_write(COLOR['red'], '%s:%s' %
#                                   (_user.nick, _user.account))
#            elif _user.is_mod:
#                _user.user_level = 3
#                self.console_write(COLOR['bright_red'], '%s:%s' %
#                                   (_user.nick, _user.account))
#            elif _user.account:
#                self.console_write(COLOR['bright_yellow'], '%s:%s' %
#                                   (_user.nick, _user.account))
#            else:
#                self.console_write(COLOR['cyan'], '%s:%s' % (_user.nick, _user.account))
#FIXED FOR BLIND USERS USING ESPEAK TO LAUNCH USE
#python2 bot.py | espeak
    def on_userlist(self, user_info):
        ...
        if user_info['handle'] != self.client_id:
            ...
            if _user.is_owner:
                _user.user_level = 1
                self.console_write(COLOR['red'], 'Joins room owner: %s:%s' %
                                   (_user.nick, _user.account))
            elif _user.is_mod:
                _user.user_level = 3
                self.console_write(COLOR['bright_red'], 'Joins moderator: %s:%s' %
                                   (_user.nick, _user.account))
            elif _user.account:
                self.console_write(COLOR['bright_yellow'], 'Joins: %s:%s' %
                                   (_user.nick, _user.account))
            else:
                self.console_write(COLOR['cyan'], 'Joins: %s:%s' % (_user.nick, _user.account))
    ...
    def on_join(self, join_info):
        ...
        if _user.account:
            ...
            if _user.is_owner:
                ...
                _user.user_level = 3
                self.console_write(COLOR['bright_red'], '%s' %
                                   (_user.nick, _user.account))
            else:
                self.console_write(COLOR['bright_yellow'], '%s %s' %
                                   (_user.nick, _user.account))
        else:
            self.console_write(COLOR['cyan'], '%s arrived.' % (_user.nick))
    ...
    def on_nick(self, uid, nick):
        ...
        old_nick = _user.nick
        _user.nick = nick
        self.console_write(COLOR['bright_cyan'], '%s:%s renamed %s' % (old_nick, uid, nick))

    def on_quit(self, nick, uid):
        _user = self.users.search(uid)
        _user.nick = nick
        self.console_write(COLOR['cyan'], '%s departed' % (nick))
        #self.console_write(COLOR['bright_green'], '%s' % (self.active_user.nick))
    ...
    def on_ban(self, ban_info):
        ...
        if ban_info['success']:
            ...
            if banned_user.account:
                self.console_write(COLOR['bright_magenta'], '%s:%s was banned' %
                                   (banned_user.nick, banned_user.account))
            else:
                self.console_write(COLOR['bright_magenta'], '%s was banned' % banned_user.nick)
    ...
    def on_publish(self, uid):
        ...
        self.console_write(COLOR['yellow'], '%s cammed up' % (_user.nick))
    ...
    def on_unpublish(self, uid):
        ...
        if _user is not None:
            ...
            self.console_write(COLOR['yellow'], '%s cammed down.' % (_user.nick))
    ...
    def on_captcha(self, key):
        ...
        self.console_write(COLOR['bright_red'], 'Captcha required! %s' % key)
