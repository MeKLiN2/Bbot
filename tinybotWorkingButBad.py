...
#### 1l1l1l1l1l1l1l1l1l1l1l1l1l1l1l1l
    ...
    def on_join(self, join_info):
        ...
        if pinylib.CONFIG.B_vip:
                ...
                print('%s denied entrance' % _user.account)
        ...
        # If account is in known.txt print, sending to chat will get kicked for spam.
            ...
            #print('%s has arrived' % _user.account)
        ...
        if _user.nick.startswith('guest-'):
            ...
            if _user.account:
                ...
                _user.user_level = 3
                self.console_write(pinylib.COLOR['bright_red'], '%s:%s' % (_user.nick, _user.account))
                if not _user.account in knownlist:
                ...
                self.console_write(pinylib.COLOR['bright_yellow'], '%s:%s' % (_user.nick, _user.account))



        if not self.user_check(_user, account=True, guest=True, nick=True, lurker=True):

        	if _user.account.startswith('meklin'):
                    self.send_chat_msg('Bot commander has entered the building.')

            #if pinylib.CONFIG.B_GREET and self.is_client_mod:

                    if not _user.nick.startswith('ChanServ'):
                        if not _user.nick.startswith('HIITSNICK1'):
                            if not _user.nick.startswith('Wittman'):
                                if not _user.nick.startswith('Wutang'):
                                    if not _user.nick.startswith('GASMAN'):
                                        if not _user.nick.startswith('MeKLiN'):
                                            if not _user.nick.startswith('guest-'):

                                                if _user.account:
                    	
                                                    self.send_chat_msg('!aa add ' + _user.account + ' verified')
                    else:
                    	#self.do_ban(_user.nick)
                        self.send_chat_msg('!aa add ' + _user.account + ' verified' % (_user.account))
                       	#self.send_chat_msg('!aa add ' + _user.account + ' verified')
