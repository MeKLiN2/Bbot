...
#blind TTS reading version of OLDEST nortbot copy which is different file tree format than New Nortbot
import json
import time
import logging
import traceback
...
#python2 bot.py | espeak
    ...
    def on_join(self, join_info):
        ...
        if _user.account:
            ...
            self.console_write(COLOR['cyan'], '%s has arrived.' % (_user.nick))
    ...
    def on_quit(self, uid,):
        ...
        if _user is not None:
            self.console_write(COLOR['bright_green'], '%s departed' % (_user.nick))
    ...
    def on_publish(self, uid):
        ...
        self.console_write(COLOR['yellow'], '%s is cam up' % (_user.nick))
    ...
    def on_unpublish(self, uid):
        ...
        if _user is not None:
            ...
            self.console_write(COLOR['yellow'], '%s cam down.' % (_user.nick))
